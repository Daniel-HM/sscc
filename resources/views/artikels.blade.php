                            ...
                            <th class="p-1 text-right">
                                <a href="{{ route('artikels', ['voorraad', ($currentOrderBy == 'voorraad.vrij' && $currentDirection == 'desc') ? 'asc' : 'desc']) }}">
                                    ...
                                    @if($currentOrderBy == 'voorraad.vrij')
                                        @if($currentDirection == 'desc') ↑ @else ↓ @endif
                                    @endif
                                </a>
                            </th>
                        </tr>
                        ...
                        @foreach ($artikels as $artikel)
                            ...
                            <tr class="even:bg-gray-900/50 odd:bg-gray-900/80">
                                <td class="p-1"><a href="{{ route('artikel', $artikel->ean) }}">{{ $artikel->omschrijving }}</a><br/><a
                                   ...
                                   <td class="p-1 text-right">@if($artikel->kassagroep){{ $artikel->kassagroep->omschrijving }}<br />@endif{{ $artikel->assortimentsgroep->omschrijving }}</td>
                                   <td class="p-1 text-right">&euro;{{ $artikel->verkoopprijs }}</td>
                                   <td class="p-1 text-center">@if($artikel->voorraad){{ $artikel->voorraad->vrij }}@else 0 @endif</td>
                            </tr>
                        @endforeach
                        </tbody>
                    </table>
